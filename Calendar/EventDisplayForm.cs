        ...
        public void DisplayEvents(DateTime date, bool monthlyMode)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connString))
                {
                    ...
                    if (monthlyMode)
                    {
                        sql = "SELECT event_id, event_name, start_date, end_date FROM tbl_calendar WHERE YEAR(start_date) = @year AND MONTH(start_date) = @month";
                    }
                    else
                    {
                        sql = "SELECT * FROM tbl_calendar WHERE DATE(start_date) = @date OR DATE(end_date) = @date OR (DATE(start_date) < @date AND DATE(end_date) > @date)";
                    }
        ...
        private void deleteEvent_Click(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                ...
                try
                {
                    ...
                    {
                        conn.Open();
                        string sql = "DELETE FROM tbl_calendar WHERE event_id = @eventId";
                        MySqlCommand cmd = new MySqlCommand(sql, conn);
