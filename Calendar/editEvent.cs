...
namespace Calendar
{
    public partial class editEvent : Form
    {
        ...
        public void editEvent_Load(object sender, EventArgs e)
        {
            ...
            dateTimePickerStartTime.Value = StartDate;
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string sql = "UPDATE tbl_calendar SET event_name = @eventName, start_date = @startDate, end_date = @endDate WHERE event_id = @eventId";
        ...
        private bool CheckForConflicts(DateTime startDate, DateTime endDate, int eventId)
        {
            try
            {
                ...
                {
                    conn.Open();
                    string sql = "SELECT COUNT(*) FROM tbl_calendar WHERE (@startDate < end_date) AND (@endDate > start_date) AND event_id != @eventId";
                    MySqlCommand cmd = new MySqlCommand(sql, conn);
                    cmd.Parameters.AddWithValue("@startDate", startDate.ToString("yyyy-MM-dd HH:mm:ss"));
                    cmd.Parameters.AddWithValue("@endDate", endDate.ToString("yyyy-MM-dd HH:mm:ss"));
                    cmd.Parameters.AddWithValue("@eventId", eventId);
